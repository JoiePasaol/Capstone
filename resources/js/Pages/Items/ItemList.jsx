...
export default function ItemList() {
    ...
    const TransferDrawer = () => {
            ...
            if (transferItemId) {
                ...
                if (item) {
                    ...
                    setTransferData((prev) => ({
                        ...prev,
                        quantityToTransfer: Math.min(
                            prev.quantityToTransfer,
        ...
        const handleTransferSubmit = async (e) => {
            ...
            try {
                ...
                setSuccessMessage(
                    response.data.message || "Item transferred successfully!"
                );
                ...
                <form onSubmit={handleTransferSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            >
                                ...
                                {signatories.map((signatory) => (
                                    <option
                                        key={signatory.id}
                                        value={signatory.name_designation}
                                    >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            >
                                <option value="">
                                    Select Office Name/Designation
                                </option>
                                {signatories.map((signatory) => (
                                    <option
                                        key={signatory.id}
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (!data.id) {
                await axios.post(route("items.store"), formData, {
            ...
            if (error.response && error.response.status === 422) {
                ...
                // Assign errors to specific input fields
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Filter out items with quantity 0
                const filteredItems = itemsResponse.data.items.filter(
                    (item) => item.quantity > 0
                );
    ...
    useEffect(() => {
        ...
        const filtered = items.filter((item) => {
            ...
            const matchesSearch =
                ...
                (item.pr && item.pr.toLowerCase().includes(searchTerm.toLowerCase())) ||
                (item.po && item.po.toLowerCase().includes(searchTerm.toLowerCase())) ||
                (item.ics && item.ics.toLowerCase().includes(searchTerm.toLowerCase())) ||
                (item.vc && item.vc.toLowerCase().includes(searchTerm.toLowerCase())) ||
                (item.ch && item.ch.toLowerCase().includes(searchTerm.toLowerCase())) ||
                (item.or && item.or.toLowerCase().includes(searchTerm.toLowerCase())) ||
                (item.property_no && item.property_no.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            await axios.post(route("items.bulk-destroy"), { ids: deleteTarget });
    ...
    const userFilteredItems = filteredItems.filter((item) => item.quantity > 0);
    ...
    const headers = [
        ...
        { label: "Total_Items", key: "quantity" },
        { label: "Available_Items", key: "remaining_quantity" },
    ...
    const rows = sortedPaginatedItems.map((item, index) => {
        ...
        let row = {
            ...
            date_purchase:
                parsedDatePurchase && !isNaN(parsedDatePurchase)
                ...
                <div className="w-full mb-3  flex justify-between items-center gap-4 flex-wrap">
                    ...
                    <div className="flex gap-2 items-center ">
                        ...
                        <div className="relative">
                            <select
                                onClick={() =>
                                    setDateRangeShowPicker(!showDateRangePicker)
                                }
                        ...
                        <div>
                            <div className="mt-2">
                                ...
                                <TextInput
                                    ...
                                    className="mt-1 block h-10 w-full rounded-sm text-sm"
                                />
                                <InputError
                                    message={errors.name}
                                    className="mt-2"
                            ...
                            <div className="mt-4">
                                ...
                                <TextInput
                                    ...
                                    onChange={(e) =>
                                        setData("property_no", e.target.value)
                                    }
                                />
                                <InputError
                                    message={errors.property_no}
                                    className="mt-2"
                                />
                            </div>
                        </div>
                        <div>
                            <div className="mt-2">
                                ...
                                <TextInput
                                    ...
                                    className="mt-1 block w-60 h-10 rounded-sm text-sm"
                ...
                rounded-sm text-sm text-gray-700 dark:text-gray-200 bg-transparent cursor-pointer flex items-center"
                            ...
                            <div className="mt-4 w-60">
                                ...
                                <div className="relative">
                                    ...
                                    rounded-sm text-sm text-gray-700 dark:text-gray-200 bg-transparent cursor-pointer flex items-center"
                                        onClick={() =>
                                            toggleDatePicker("date_purchase")
                                        }
                                    >
                                        <option hidden value="">
                                            {data.date_purchase
